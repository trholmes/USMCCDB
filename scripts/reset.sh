...
# Same resolution compose uses: an explicit COMPOSE_PROJECT_NAME (from
...
if compose ps --status running backup >/dev/null 2>&1 && \
   compose ps --status running backup | grep -q backup; then
...
"$ENGINE" volume rm "${project}_pgdata"
echo
